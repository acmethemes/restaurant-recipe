 ...
 */
function restaurant_recipe_sanitize_social_data( $input ){
	...
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				...
				elseif ( $key == 'checkbox' ){
					$input_decoded[$boxes][$key] = restaurant_recipe_sanitize_checkbox( $value );
                }
                else{
 ...
 * @return string json encoded
 */
function restaurant_recipe_sanitize_slider_data( $input ){
	$input_decoded = json_decode( $input, true );
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				if( $key == 'selectpage'){
					$input_decoded[$boxes][$key] = restaurant_recipe_sanitize_page( $value );
				}
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Restaurant_Recipe_Customize_Icons_Control' )):
	/**
	 ...
	 *
	 */
	class Restaurant_Recipe_Customize_Icons_Control extends WP_Customize_Control {
            ...
            <label>
                ...
                <span class="at-customize-icons">
                    ...
                    <span class="icon-toggle">
                        <?php echo ( empty( $value )? esc_html__('Add Icon','restaurant-recipe'): esc_html__('Edit Icon','restaurant-recipe') ); ?>
                        <span class="dashicons dashicons-arrow-down"></span>
                    </span>
                    <span class="icons-list-wrapper hidden">
                        <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','restaurant-recipe')?>">
	                    <?php
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Restaurant_Recipe_Repeater_Control' )):
	/**
	 ...
	 *
	 */
	class Restaurant_Recipe_Repeater_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		...
		public function enqueue(){
			...
			wp_enqueue_style( 'restaurant-recipe-customizer-repeater-style', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.css', array(), '3.3.6' );
			wp_enqueue_script( 'restaurant-recipe-customizer-repeater-script', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.js', array('jquery', 'jquery-ui-draggable' ), '1.0.0', true  );
		}
                    ...
                    <div class="repeater-fields hidden">
								...
								switch ( $field['type'] ) {
									...
									case 'icons':
                                        ...
                                        <span class="at-customize-icons">
                                            ...
                                            <span class="icon-toggle">
                                                <?php esc_html_e('Add Icon','restaurant-recipe'); ?>
                                                <span class="dashicons dashicons-arrow-down"></span>
                                            </span>
                                            <span class="icons-list-wrapper hidden">
                                                <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','restaurant-recipe')?>">
									...
									case 'image':
                                        ...
                                        <span class="img-preview-wrap" <?php echo  $restaurant_recipe_display_none ; ?>>
                                            <img class="widefat" src="<?php echo esc_url( $new_value ); ?>" alt="<?php esc_attr_e( 'Image preview', 'restaurant-recipe' ); ?>"  />
                                        ...
                                        <input type="button" value="<?php esc_attr_e( 'Upload Image', 'restaurant-recipe' ); ?>" class="button media-image-upload" data-title="<?php esc_attr_e( 'Select Image','restaurant-recipe'); ?>" data-button="<?php esc_attr_e( 'Select Image','restaurant-recipe'); ?>"/>
                        ...
                        <div class="clearfix repeater-footer">
                            <a class="repeater-field-remove" href="#remove">
								<?php esc_html_e( 'Delete', 'restaurant-recipe' ); ?>
                            </a>
	                        <?php esc_html_e( '|', 'restaurant-recipe' ); ?>
                            <a class="repeater-field-close" href="#close">
								<?php esc_html_e( 'Close', 'restaurant-recipe' ); ?>
                            </a>
                        </div>
                    </div>
                </li>
									...
									switch ( $field['type'] ) {
										...
										case 'icons':
                                            ...
                                            <span class="at-customize-icons">
                                                ...
                                                <span class="icon-toggle">
                                                    <?php echo ( empty( $new_value )? esc_html__('Add Icon','restaurant-recipe'): esc_html__('Edit Icon','restaurant-recipe') ); ?>
                                                    <span class="dashicons dashicons-arrow-down"></span>
                                                </span>
                                                <span class="icons-list-wrapper hidden">
                                                    <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','restaurant-recipe');?>">
